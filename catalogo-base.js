...
export const catalogoBase = {
  ...
  CERVEJAS: [
    [
      "CERVEJA HEINEKEN 269ML (ZERO)",
      "cerveja-heineken-zero-lata-269ml-1.png",
      600,
      12,
    ],
  ...
  "REFRIGERANTES E OUTROS": [
    [
      "MONSTER 473ML ENERGY",
      "energetico-monster-energy-lata-473ml-1.png",
      1000,
      6,
    ],
    [
      "MONSTER 473ML ENERGY (ZERO)",
      "energetico-monster-energy-lata-473ml-1.png",
      1000,
      6,
    ],
    [
      "REFRIG ANTARCTICA 350ML GUARANÁ",
      "refrigerante-guarana-antarctica-350ml-1.png",
      500,
      12,
    ],
    [
      "REFRIG ANTARCTICA 350ML (ZERO)",
      "refrigerante-guarana-antarctica-350ml-1.png",
      500,
      0,
    ],
...
export const slugProduto = (nome) =>
  nome
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
export const imagemProduto = (imagem) =>
  /^(https?:)?\/\//i.test(imagem) ? imagem : pastaImagens + imagem;
export function produtosIniciais() {
  return Object.fromEntries(
    Object.entries(catalogoBase).flatMap(([category, items]) =>
      items.map(([name, image, priceCents, stock]) => {
        const id = slugProduto(name);
        return [
          id,
          { name, category, image, enabled: stock > 0, priceCents, stock },
        ];
      }),
    ),
  );
