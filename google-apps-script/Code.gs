function doPost(e) {
  ...
  try {
    ...
    aba.getRange(aba.getLastRow(),6).setNumberFormat('[$R$-pt-BR] #,##0.00');
...
function obterAbaPedidos(planilha) {
  ...
  if (aba.getLastRow() === 0) {
    aba.getRange(1,1,1,6).setValues([["Data","Hora","Nome","Bebidas","Pago","Valor total"]]);
    aba.getRange("A1:F1").setFontWeight("bold").setBackground("#1f4e78").setFontColor("#ffffff");
    aba.setFrozenRows(1);
    aba.autoResizeColumns(1,6);
  }
  const cabecalhos = aba.getRange(1,1,1,Math.max(1,aba.getLastColumn())).getValues()[0];
  const colunaPedidoId = cabecalhos.indexOf("Pedido ID") + 1;
  if (colunaPedidoId) aba.deleteColumn(colunaPedidoId);
  aba.getRange(1,1,1,6).setValues([["Data","Hora","Nome","Bebidas","Pago","Valor total"]]);
  aba.getRange("A1:F1").setFontWeight("bold").setBackground("#1f4e78").setFontColor("#ffffff");
  aba.getRange("F:F").setNumberFormat('[$R$-pt-BR] #,##0.00');
  aba.autoResizeColumns(1,6);
  return aba;
}
